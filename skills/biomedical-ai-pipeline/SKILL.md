...
Código: `TargetIdentificationEngine`, `MoleculeGenerationEngine`, `AgingClocksValidator`, `DoseDissociationAnalyzer`, `UKBiobankClient` → `references/implementation.md`.

## Uso Rápido

```python
targets = TargetIdentificationEngine().identify_targets("idiopathic pulmonary fibrosis")
mols = MoleculeGenerationEngine().generate_candidates(targets[0], num_candidates=100)
result = AgingClocksValidator().validate_treatment(pre, post, placebo_pre, placebo_post, doses)
assert result.consensus  # todos os 6 relógios concordam?
```
...
| Skill | Relação |
|---|---|
| `api-connector-builder` | Conectores PubMed, UK Biobank, clinical trials |
...
- `references/implementation.md` — engines, validator, dose analysis, UKB client
- [In Silico](https://www.insilico.com) · [Nature Biotech](https://www.nature.com/nbt) · [UK Biobank](https://www.ukbiobank.ac.uk)
- Video: `l4EJUm6KwM0.pt.dedup.txt` — linhas 22-75, 106-180, 206-230

...
Track per run: bootup overhead (messages before first output), message-drop
rate, dead agents (`?`/stall/timeout), abandoned threads, claim violations,
referee rejection rate. If bootup overhead dominates, the swarm is too large
for the task — shrink it or switch to orchestration.

## Anti-Patterns

- Canonical written collaboratively in chat → deadlock.
- Shared file without lock → silent overwrites.
- No bail-out exit → rule-breaking, sandbox escape attempts.
- Vibe-coded swarm without sandboxing → run everything sandboxed; the sandbox
  is the last line of defense, not an optimization.

## Relations

- `sessoes-orquestradas`, `team-agent-orchestration`, `fusion-harness`,
  `workflows`, `engenharia-de-grafos`: orchestrated delegation (planner exists).
- `agent-guardrails` + G20: impossible-task handling, monitor design.
- `enterprise-agent-ops`: sandbox-as-last-line.
- `cost-aware-llm-pipeline`: budget ceilings per thread.

## Sources

- Pattern from IndyDevDan "Are Agent Swarms USEFUL?" (2026-09-07,
  `S2sjyokoxeE`): mailbox/thread, claim-file, check-budget, done+bail-out,
  referee/canonical, coordination metrics, sandbox-last-line.
- Model-specific run numbers in that video (GLM/DeepSeek/Gemini costs) are
  anecdotal author measurements, not benchmarks — do not quote as facts.
- Independent swarm-failure analyses (Meter, Redwood Research, OpenAI Astra
  incident post) cited by the video but not verified here — confirm before
  citing in customer-facing material.

...
- Bot argues with the user instead of handing off -> lost lead.
- Old hosted tool kept running on the same keywords -> double DM.

## Relations

- `social-publisher`: owns scheduled public posts that carry the trigger keywords.
- `whatsapp-evolution-go`: owns WhatsApp-side delivery when the DM must move to WhatsApp.
- `ads-operator`: owns paid account changes; this skill only tracks UTM, never touches ad accounts.

## Sources

- Messenger Platform messaging docs (send API, webhooks): https://developers.facebook.com/docs/messenger-platform/
- Meta Marketing API root (campaign structure reference): https://developers.facebook.com/docs/marketing-api
- Funnel shape (keyword -> DM -> UTM -> pixel) and the "ManyChat replacement" claim from video evidence #158 are author measurement, not benchmarks - re-measure sends, replies, and completions in your own account before quoting rates.

...
- `dailyTriageSOP` (LLM): fetch → heuristic-filter → vision-eval (parallel) → rank → report → improve

## Checklist de Entrega

- [ ] `scripts/code-as-graph.js` — engine code-as-graph
- [ ] `scripts/llm-as-graph.js` — engine LLM-as-graph
- [ ] `scripts/state-management.js` — state + I/O schemas
- [ ] `scripts/verifier-planner.js` — verifier/planner separation
- [ ] `scripts/io-schemas.js` — schemas de todos node types
- [ ] `scripts/templates.js` — ship-change, daily-triage (code + SOP)
...
- [AI Jason Video](https://www.youtube.com/watch?v=_9OT25ZvrWs) · [Dynamic Workflow](https://docs.anthropic.com/en/docs/claude-code/dynamic-workflow) · [Hooks](https://docs.anthropic.com/en/docs/claude-code/hooks) · [Building Effective Agents](https://www.anthropic.com/engineering/building-effective-agents)
